<template>
    <div>
        <div class="container-fluid">
            <div class="row">
                <div class="col">
                    <h5 class="mx-auto text-sm-h6 text-wrap text-center mt-2">Разделение зарегистрированных счетчиков по производителю</h5>
                    <apex-chart
                        v-show="manufacturesSeries"
                        type="pie"
                        ref="pieManufacturesChart"
                        :options="programmingManufacturesOptions"
                        :series="manufacturesSeries"
                        height="350"
                    />
                </div>
            </div>
...
<script>
	...
	import programmingManufacturesOptions from "../js/programming-not-in-pyramid-types-options"
	...
	export default {
		...
		data: () => ({
			...
			programmingManufacturesOptions,
			typeSeries: [],
			manufacturesSeries: [],
		...
		mounted() {
			...
			this.getMeterRegistrationManufacturesChartSeries()
		},
		methods: {
			...mapActions('charts',
				[
					...
					'getMeterRegistrationManufacturesChartData',
				]),
			...
			async getMeterRegistrationManufacturesChartSeries() {
				try {
					const data = await this.getMeterRegistrationManufacturesChartData()
					const { series, labels } = this.getDataChartSeries(data)
					this.manufacturesSeries = series
					this.$refs.pieManufacturesChart?.updateOptions({ labels: labels })
				} catch (e) {
					this.showNotificationRequestError(e)
				}
			},
