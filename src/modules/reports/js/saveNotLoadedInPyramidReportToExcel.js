...
export default function saveNotLoadedInPyramidReportToExcelFile(data) {
    ...
    tableData.push(['Серийный номер', 'Тип', 'Принадлежность', 'Сим карта', 'Дата регистрации', 'На складе'])
    tableData = tableData.concat(data)

    workBook.SheetNames.push("Не загруженные");

    //let workSheet = XLSX.utils.aoa_to_sheet(tableData, { cellDates: true })
    let workSheet = XLSX.utils.aoa_to_sheet(tableData, { cellDates: true });
    ...
    XLSX.writeFile(workBook, 'pyramid-not-loaded.xlsx')
